        ...
        <form class="form-control bg-light p-4" action="{{ route('admin.dishes.store') }}" method="post"
            ...
            <div class="mb-3">
                ...
                <input type="number" step="0.01" class="form-control @error('price') is-invalid @enderror" name="price"
                    id="price" aria-describedby="priceHelper" placeholder="price" value="{{ old('price') }}" />
                @error('price')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="btn-group" role="group" aria-label="Basic checkbox toggle button group">
                <input type="radio" class="btn-check" name="visibility" id="visibility" autocomplete="off" value="1" />
                <label class="btn btn-outline-primary" for="visibility">Visible</label>

                <input type="radio" class="btn-check" name="visibility" id="visibility" autocomplete="off" value="0" />
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control" name="ingredients" id="ingredients" rows="5" >{{ old('ingredients') }}</textarea>
            </div>
